...
The phone numbers and e-mail addresses are read from the eGroupware database table B<contacts> and 
...
Further, eGroupware had build-in functionality called B<lazy update> to reduce re-writing the address 
books of the supported clients as much as possible. This reduces CPU time but - more important - also 
...
write cycles. But because F<egw2fbox.pl> avoids unnecessary write cycles as much as possible, it can 
safely be used.
